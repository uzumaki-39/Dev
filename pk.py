...
import time
import logging
from typing import Optional, Dict, List, Any  # <── Already present
...
# ── Config ──────────────────────────────────────────────────────────────────
TOKEN = "8901668516:AAGrIt__UkH6gEGLQos6iHxv13yp6AVkFkw"  # Replace with your bot token
...
def generate_payment_link(
    ...
    try:
        ...
        return {
            ...
            "amount": amount,  # <── FIXED: Added missing amount field
            "active": link.active,
...
async def cmd_help(message: types.Message):
    text = (
        ...
        f"{pe(E['list'])} /intents sk_xxx [limit] — {bold('List Payment Intents')}\n\n"
...
async def cb_menu_cmds(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"{pe(E['list'])} /intents sk [limit] — {bold('List Intents')}"
    )
