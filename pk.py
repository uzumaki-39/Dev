...
# ── Config ──────────────────────────────────────────────────────────────────
TOKEN = "8901668516:AAGrIt__UkH6gEGLQos6iHxv13yp6AVkFkw"  # <── REPLACE THIS WITH YOUR ACTUAL TOKEN
...
def lookup_key_info(sk: str) -> Dict[str, Any]:
    ...
    FIXED: Handles both old and new Stripe API versions.
    """
    try:
        # ── Use the legacy stripe module for compatibility ──
        ...
        try:
            ...
            log.warning(f"Balance lookup failed: {e}")
        
        # ── Get account info (try multiple approaches) ──
        account = None
        
        # Approach 1: Try the new StripeClient method
        try:
            client = StripeClient(sk)
            account = client.accounts.retrieve()
        except TypeError:
            # Approach 2: Try the old stripe.Account.retrieve() with no args
            try:
                account = stripe_lib.Account.retrieve()
            except TypeError:
                # Approach 3: Try with the account ID from the key
                try:
                    # Some keys have the account ID embedded
                    account = stripe_lib.Account.retrieve()
                except Exception:
                    account = None
            except Exception:
        ...
        try:
            ...
            log.warning(f"Transaction list failed: {e}")
    ...
    return {
        "inline_keyboard": [
            ...
            [
                ...
                {"text": f"{bold('Intents')}", "callback_data": "menu_intents", "style": "secondary"},
            ],
        ]
    }
...
# ── Start Command (FIXED with both CommandStart and Command fallback) ──
@router.message(CommandStart())
async def cmd_start(message: types.Message):
    await message.reply(WELCOME_MSG, reply_markup=menu_keyboard())


@router.message(Command("start"))  # <── FALLBACK for /start
